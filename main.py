...
def get_oeo_key_from_db():
    ...
    keys_exists = db.session.execute(query_test).fetchone()
    keys_available = db.session.execute(query_get_creds).fetchone()
...
def run_analysis(osm_id=None, wq_feature=None, model_id=None):
    ...
    if isinstance(oeo_ckeys, tuple):
        ...
        try:
            ...
            # conn = openeo.connect(OPENEO_BACKEND, auto_validate=True)
            # conn.authenticate_oidc_client_credentials(provider_id=OPENEO_PROVIDER, client_id=clid, client_secret=clse)

            # List jobs
            # jobs = conn.list_jobs()
            # pd_jobs = pd.DataFrame(jobs)
            # pd_jobs.to_csv("jobs.csv")
            ...
            if osm_id:
                ...
                aihabs.db_table_S2_points_data = db_s2_points
    ...
    return oeo_ckeys    # TODO: proč vrací klíče?
...
def select_waterbody():
    ...
    lat = lyr_point_position['lat']
    lon = lyr_point_position['lng']
    ...
    print(f"Checking the reservoir {reserv_name} ({osm_id}) in the DB...")
...
def get_osm_geometry(env_path, osm_service_path, lat, lon, dist=20):
    """Obtaining the water reservoir polygon from osmnx using external service"""
    
    try:
        result = subprocess.run([env_path, osm_service_path, str(lat), str(lon), str(dist)],
            capture_output=True,
            text=True,
            check=True)    
...
def data_info():                                # TODO: dodělat! --> přidat odkaz na použitý model
    ...
    # Get the name for the reservoir for which is the model valid
    if osm_id_model:
        query_model_wr = text(f"SELECT name FROM {water_reservoirs} WHERE osm_id = '{osm_id_model}'")
        df_model_wr = pd.read_sql_query(query_model_wr, db.engine)
        if not df_model_wr.empty:
            osm_id_model_name = df_model_wr.iloc[0, 0]
            wr_name_model = osm_id_model_name
        else:
            wr_name_model = "Unknown"
    ...
    if not osm_id_model:
        ...
        if is_default:
            osm_id_model = wr_name_model + " (default)"
        else:
            osm_id_model = wr_name_model
    ...
    data = [
        ...
        {'info': '', 'val1': 'Test accuracy:', 'val2': str(round(test_accuracy, 2))},
...
def get_wqfeature_info():
    """
    Get the info about water quality features from the DB and add the popover info.
    """
    ...
    print(f"Getting the info for the water quality feature: {feature}")

    # Get the list of water quality features from the DB
    query = text(f"SELECT wq_descr FROM wq_info WHERE wq_feature = '{feature}'")  # Get the wq feature description from the DB
    df_data = pd.read_sql_query(query, db.engine)
    if df_data.empty:
        return jsonify({"error": "No data found. Select the water quality feature first."}), 404
    wq_descr = df_data.iloc[0, 0]


    # Prepare the data for the popover
    data = {
        'feature': feature,
